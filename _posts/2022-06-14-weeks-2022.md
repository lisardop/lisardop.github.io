...
*These are the matching results with old datapump. As we can observe, Mendoza glyph matchs with itself, but Matrícula de Tributos as light-colored and vanished matches with a non-color, light-brown full background glyph*
...
*As we can see, none of them match the examples of Ahuatepec or Ahuatl de Mendoza. Transparent or paper-colored backgrounds are most accurate when the glyph has no artificial color. When artificially colored, the glyph matches other trees and plants in a 'cross' shape. Some water elements are present. Trees and tree-like plants are very difficult to distinguish, even for human eye due to the high complexity of peripheral elements, resulting in different names of trees or species.*
...
This week Stephanie Wood send me a new datapump stored by iconography, simplex, compound and atomic glyphs. Ignoring those repeated, there are 2012 image files, plus 62 digital painted glyphs.
...
*First ocelote head with a white background seems to be the most accurated. All its results are related with ocelote, but 2nd one is Ehecatl face.*

Meanwhile, I prepare the new datapump from Mendoza Codex to upload. As soon as I finnish Matricula de Tributos, all the files will be uploaded too.
